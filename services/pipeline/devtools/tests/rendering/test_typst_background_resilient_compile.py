...
def test_background_render_resilient_compile_sanitizes_on_failure() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with mock.patch(
            "retainpdf_pipeline.services.rendering.output.typst.book_renderer.compile_typst_render_pages_pdf",
            side_effect=[RuntimeError("mitex failed"), root / "probe.pdf", root / "sanitized.pdf"],
        ) as compile_mock, mock.patch(
            "retainpdf_pipeline.services.rendering.output.typst.book_renderer.collect_background_page_specs",
            return_value=[(0, 200.0, 300.0, translated_pages[0])],
        ), mock.patch(
            "retainpdf_pipeline.services.rendering.output.typst.book_renderer.sanitize_page_specs_for_typst_book_background",
...
def test_background_render_color_adapt_samples_original_pdf_not_cleaned_background() -> None:
    from retainpdf_pipeline.services.rendering.output.typst.book_renderer import _apply_background_page_color_adapt
    from retainpdf_pipeline.services.rendering.output.typst.emitter import build_typst_source_from_page_specs
