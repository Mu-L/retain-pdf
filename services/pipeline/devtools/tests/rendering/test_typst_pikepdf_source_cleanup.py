...
from retainpdf_pipeline.services.rendering.document.pikepdf_overlay import PikepdfOverlayResult
...
from retainpdf_pipeline.services.rendering.output.typst.overlay_chunk_compile import OverlayChunkCompileResult
...
def test_large_pikepdf_overlay_uses_chunked_typst_compile_when_enabled(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            with mock.patch(
                "retainpdf_pipeline.services.rendering.output.typst.overlay_ops.compile_book_overlay_pdf",
                side_effect=AssertionError("large pikepdf overlay should use chunked compile"),
            ), mock.patch(
                "retainpdf_pipeline.services.rendering.output.typst.overlay_ops.compile_book_overlay_pdf_chunks",
                chunk_compile_mock,
            ), mock.patch(
                "retainpdf_pipeline.services.rendering.output.typst.overlay_ops.overlay_pdf_chunks_with_pikepdf",
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            with mock.patch(
                ...
                "retainpdf_pipeline.services.rendering.output.typst.overlay_ops.overlay_pdf_pages_with_pikepdf",
                return_value=PikepdfOverlayResult(
                    output_pdf_path=output_pdf,
                    pages_merged=1,
                    elapsed_seconds=0.1,
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            with mock.patch(
                ...
                "retainpdf_pipeline.services.rendering.output.typst.page_compile.compile_page_overlay_pdf",
                return_value=overlay_pdf,
            ), mock.patch(
                "retainpdf_pipeline.services.rendering.output.typst.overlay_book.apply_source_page_overlay",
