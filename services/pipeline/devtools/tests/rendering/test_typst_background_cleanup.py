...
from retainpdf_pipeline.services.rendering.output.typst.book_renderer import _background_cache_key
...
def test_background_stage_uses_cover_only_redaction_for_vector_text() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with mock.patch(
            "retainpdf_pipeline.services.rendering.source.background.stage.collect_vector_text_rects",
            return_value=[fitz.Rect(10, 20, 80, 60)],
        ), mock.patch(
            "retainpdf_pipeline.services.rendering.source.background.stage.redact_source_text_areas",
        ) as redact_mock, mock.patch(
            "retainpdf_pipeline.services.rendering.source.background.stage.save_optimized_pdf",
        ):
...
def test_background_stage_skips_old_cleanup_for_precleaned_pages() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with mock.patch(
            "retainpdf_pipeline.services.rendering.source.background.stage.protect_formula_regions_in_redaction_items",
...
def test_apply_source_page_overlay_uses_cover_only_when_vector_text_detected() -> None:
    ...
    with mock.patch(
        "retainpdf_pipeline.services.rendering.source.background.redaction_plan.collect_vector_text_rects",
        return_value=[fitz.Rect(10, 20, 80, 60)],
    ), mock.patch(
        "retainpdf_pipeline.services.rendering.source.background.source_overlay.redact_source_text_areas",
    ) as redact_mock, mock.patch(
        "retainpdf_pipeline.services.rendering.source.background.source_overlay.strip_page_links",
    ):
