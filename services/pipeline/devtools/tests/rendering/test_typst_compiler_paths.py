...
def test_typst_compile_error_carries_structured_context() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        work_dir = Path(tmp)
        with mock.patch("retainpdf_pipeline.services.rendering.output.typst.compiler.subprocess.run", return_value=completed):
...
def test_typst_compile_error_wraps_runtime_start_failure() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        work_dir = Path(tmp)
        with mock.patch(
            "retainpdf_pipeline.services.rendering.output.typst.compiler.subprocess.run",
...
def test_book_overlay_compile_falls_back_when_prebuilt_source_is_missing() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with mock.patch("retainpdf_pipeline.services.rendering.output.typst.compiler.subprocess.run", return_value=completed) as run_mock:
