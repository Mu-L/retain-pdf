...
def results(report_path, report):
    ...
    items = {}
    pages = sorted(root.glob("page-*-deepseek.json"))
    for page in pages:
        for item in json.loads(page.read_text()):
            if item["item_id"] in items:
                raise ValueError("duplicate item ID")
            items[item["item_id"]] = item
    if not pages or not (root / "translation-manifest.json").is_file():
        raise ValueError("missing completed translation artifacts")
...
def evaluate(baseline_path, candidate_path):
    ...
    old, old_pages = results(baseline_path, a)
    new, new_pages = results(candidate_path, b)
    errors = []
