...
export function isReaderTransportError(error: unknown): error is ReaderTransportError {
  return Boolean(error && typeof error === "object" && (
    "code" in error || "status" in error
  ));
}

export function createReaderTransportError(
  message: string,
  status?: number,
  code?: string,
): ReaderTransportError {
  const error = new Error(message) as ReaderTransportError;
  error.name = "ReaderTransportError";
...
export type ReaderLiveTranslationPort = {
  fetchLayout: (
    jobId: string,
    options?: ReaderLiveTranslationRequestOptions,
  ) => Promise<ReaderLiveTranslationLayout>;
  fetchPage: (
    jobId: string,
    pageIdx: number,
    options?: ReaderLiveTranslationRequestOptions,
  ) => Promise<ReaderLiveTranslationPageSnapshot>;
  streamEvents: (
    jobId: string,
    options: ReaderLiveTranslationStreamOptions,
  ) => Promise<void>;
};
