...
- `frontend/web/src/app/reader/entry.tsx` 注册 RetainPDF adapters 后调用 `@retainpdf/reader/boot`。
...
`frontend/web/dist/css/` 现在只保留当前构建产物 `home.css`、`detail.css`、`reader.css`，不再有 `reader-legacy.css`。
...
find frontend/web/src/app/reader -type f | sort
