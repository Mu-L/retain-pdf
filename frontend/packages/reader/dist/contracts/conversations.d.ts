...
export type ReaderConversationDetail = ReaderConversationRecord & {
    messages: ReaderMessageRecord[];
};
...
export type ReaderConversationPort = {
    create: (payload?: {
        title?: string;
        document_id?: string;
    }) => Promise<ReaderConversationRecord>;
    list: (query?: {
        limit?: number;
        offset?: number;
        document_id?: string;
    }) => Promise<{
        conversations: ReaderConversationRecord[];
    }>;
    get: (conversationId: string) => Promise<ReaderConversationDetail>;
    delete: (conversationId: string) => Promise<{
        deleted: boolean;
    }>;
    patch: (conversationId: string, payload: {
        head_id?: string;
        title?: string;
    }) => Promise<ReaderConversationRecord>;
    appendMessage: (conversationId: string, payload: Record<string, unknown>) => Promise<ReaderMessageRecord>;
    forkFromPath: (options: {
        documentId?: string;
        title?: string;
        path: ReaderConversationPathItem[];
    }) => Promise<{
        conversation: ReaderConversationRecord;
        items: Array<{
            parentId: string | null;
            message: any;
        }>;
