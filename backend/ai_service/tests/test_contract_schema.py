"""生产者契约测试:ai_service 的实现必须与 backend/contracts/ai-ask.v1.schema.json 一致。
...
def test_ask_input_fields_match_contract():
    ...
    # question 的约束与契约一致
    q = AskInput.model_fields["question"]
    props = schema["properties"]["question"]
    metadata = {type(m).__name__: m for m in q.metadata}
    assert props["minLength"] == 1 and props["maxLength"] == 4000


def test_sse_event_types_match_contract():
    """app.py 实际发出的 SSE type 必须 ⊆ 契约枚举(枚举可超前,实现不可越界)。"""
    schema = _load_contract()["definitions"]["SseEventType"]["enum"]
    app_source = (Path(__file__).resolve().parents[1] / "retainpdf_ai" / "app.py").read_text(
        encoding="utf-8"
    )
    import re

    emitted = set(re.findall(r'"type":\s*"([a-z_]+)"', app_source))
    # compress 事件在 memory 模块组装
    compress_source = (
        Path(__file__).resolve().parents[1] / "retainpdf_ai" / "memory" / "compress.py"
    ).read_text(encoding="utf-8")
    emitted |= set(re.findall(r'"type":\s*"([a-z_]+)"', compress_source))
...
def test_done_payload_required_fields_are_produced():
    ...
    # 直接静态对照 app.py 的 payload 构造:answer/citations/tool_trace/rounds/persisted
    app_source = (Path(__file__).resolve().parents[1] / "retainpdf_ai" / "app.py").read_text(
        encoding="utf-8"
    )
    for field in required:
        assert f'"{field}"' in app_source, f"_result_payload 未产出契约 required 字段 {field}"
    del result
