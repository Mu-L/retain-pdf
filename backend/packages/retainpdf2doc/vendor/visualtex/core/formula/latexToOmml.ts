...
import { browserXmlRuntime, type VisualTexXmlRuntime } from "../xmlRuntime.ts";
import { normalizeMathLiveCanonicalUprightCommands } from "../compat/normalizeChineseLatex.ts";
import {
  MATHJAX_INTEGRAL_OPERATOR_CHARACTERS,
  normalizeMathJaxUnsupportedNaryCommands,
} from "../compat/mathJaxCompatibility.ts";
import {
  assertResolvedPresentationMathMl,
  VISUALTEX_MATHML_MACROS,
} from "../compat/latexCompatibility.ts";
import {
...
function parseMathMl(
  ...
  xmlRuntime: VisualTexXmlRuntime,
) {
  const mathMl = latexToMathMl(latex, displayMode);
  const documentObject = xmlRuntime.parse(mathMl);
  const parseError = Array.from(documentObject.getElementsByTagName("parsererror"))[0];
  if (parseError) {
    throw new Error(
...
export function latexLinesToOmml(
  ...
  xmlRuntime: VisualTexXmlRuntime = browserXmlRuntime,
) {
  const normalized = normalizeLines(lines);
  const alignRelations = relationAlignedCodeFormat(codeFormat);
  const converted = normalized.map((line) => {
    const mathElement = parseMathMl(line, displayMode, xmlRuntime);
