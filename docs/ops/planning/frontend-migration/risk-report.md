...
| **Pure logic never rewritten** | `docs/ops/planning/frontend-migration/legacy-audit.md`: 64 live VMs in `job/` + `job-status/` + `status-detail/` are **copy, not rewrite** into `packages/domain`. Their tests (`job-normalize.test.mjs` etc.) move verbatim. Risk = 0. |
