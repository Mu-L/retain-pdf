// Theme runtime API — ported from apps/web/src/shared/theme/theme.ts
import {
  DEFAULT_THEME_ID,
  THEME_STORAGE_KEY,
  getThemeDefinition,
...
export function getTheme(): ThemeId {
  if (typeof document !== "undefined") {
    const fromDom = (document.documentElement as HTMLElement).dataset.theme
    if (isThemeId(fromDom)) return fromDom as ThemeId
  }
...
export function setTheme(theme: ThemeId) {
  ...
  if (typeof document !== "undefined") {
    ;(document.documentElement as HTMLElement).dataset.theme = next
    const def = getThemeDefinition(next)
    ;(document.documentElement as HTMLElement).dataset.themeGroup = def?.group || "light"
