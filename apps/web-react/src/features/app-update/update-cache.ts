/**
 * Update check cache — ported from apps/web/src/js/features/app-update/state.ts
 * Uses localStorage key retainpdf:update-check:v1 with 24h TTL.
 */
...
function normalizeCachedInfo(value: any) {
  if (!isObject(value)) return null
  const checkedAt = Number(value.checkedAt)
  const latestVersion = `${value.latestVersion || ""}`.trim()
  if (!Number.isFinite(checkedAt) || !latestVersion) return null
  return {
    checkedAt,
...
export function createUpdateCachePort({
  ...
  function read() {
    try {
      ...
      if (!cached) return { info: null, fresh: false }
      const ageMs = now() - cached.checkedAt
      return { info: cached, fresh: ageMs >= 0 && ageMs < CACHE_TTL_MS }
    } catch {
      return { info: null, fresh: false }
    }
  }

  function write(info: any) {
    if (!info) return
    try {
      const cached = {
        checkedAt: now(),
      ...
      // cache failures never break update checks
    }
  }
...
export function writeUpdateCache(info: unknown, now = Date.now()) {
  createUpdateCachePort({ now: () => now }).write(info as any)
}
