...
// 消费侧: apps/web src/js/job/types.ts & src/js/job-status/types.ts & src/js/job/normalize.ts
...
const jobTypesSrc = readFileSync(join(process.cwd(), "src/js/job/types.ts"), "utf8");
const jobStatusTypesSrc = readFileSync(join(process.cwd(), "src/js/job-status/types.ts"), "utf8");
...
test("JobStageSnapshotView: display_stage/stage/substage/lane/stage_detail/progress 完整", () => {
  ...
  const adapterSrc = readFileSync(join(process.cwd(), "src/js/job-status/job-stage-contract-adapter.ts"), "utf8");
...
test("JobStagesView: ocr/translation/render 三阶段齐全且前端可见", () => {
  ...
  // 前端 stage 引擎映射依赖
  const engineSrc = readFileSync(join(process.cwd(), "src/js/job-status/public-stage-engine.ts"), "utf8");
...
test("JobListItemView: 列表卡关键字段与契约一致，且前端 normalize 覆盖", () => {
  ...
  const normalizeSrc = readFileSync(join(process.cwd(), "src/js/job/normalize.ts"), "utf8");
...
test("Stage progress bundle: 前端进度文案与契约 progress 单测对齐", () => {
  ...
  const progressSummarySrc = readFileSync(join(process.cwd(), "src/js/job-status/job-status-summary-progress.ts"), "utf8");
